...
function About() {
  ...
  return(
    <React.Fragment>
      <div className="w-full mx-auto py-10 bg-stone-200 dark:bg-slate-800 dark:text-zinc-300">
        ...
        <div className="mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-12 py-5 uppercase">
          {Object.values(emp).map((tile) =>(
            <div className="flex w-80 md:w-[28rem] m-auto whitespace-pre">
              {tile.id % 2 === 0 || tile.id === 0 ? 
                <div className="w-full grid grid-cols-2 gap-1">
                  ...
                  <p>{tile.name}</p>
                  <p className="text-sm whitespace-normal">{tile.job}</p>
                  </div>
                </div>
              :
                <div className="w-full grid grid-cols-2 gap-1">
