...
function About() {
  ...
  return(
    <React.Fragment>
      <div className="w-full mx-auto py-10 bg-stone-200 dark:bg-gray-800 dark:text-zinc-300">
        <div className=" w-full md:w-3/4 mx-auto">
          ...
          <div className="mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 py-12 w-5/6 uppercase">
            {Object.values(emp).map((tile) =>(
              <div className="flex w-[19rem] md:w-[30rem] m-auto whitespace-pre">
                {tile.id % 2 === 0 || tile.id === 0 ? 
                  <div className="w-full grid grid-cols-3 gap-1">
                    <img src={tile.img} alt={`image of ${tile.name}`} className='w-36 h-24 md:h-28 m-auto inline-block text-wrap px-4 border-r-2 dark:border-slate-300'/>
                ...
                <div className="w-full grid grid-cols-3 gap-1">
                    ...
                    <img src={tile.img} alt={`image of ${tile.name}`} className='w-36 h-24 md:h-28 m-auto inline-block text-wrap px-4 border-l-2 dark:border-slate-300'/>
                  </div>
                }
